...
- **노출 삼각형(Exposure Triangle)** — ISO·조리개·셔터속도 세 가지가 서로 트레이드오프(하나를 올리면 다른 하나를 낮춰야 하는 관계)를 이루는 관계. → [노출의 기본](../02-fundamentals/exposure-basics.md)
...
- **신월 / 초승달** — 신월은 달이 태양과 같은 방향에 있어 거의 보이지 않는 상태(삭이라고도 함), 초승달은 신월 직후 가늘게 뜨는 달. 달빛이 약할수록 은하수가 잘 보인다. → [은하수 찾기와 타이밍](../02-fundamentals/finding-the-milkyway.md)
...
- **이슬점 / 결로** — 이슬점은 공기 중 수증기가 물방울로 맺히기 시작하는 온도. 기온이 이 온도 아래로 내려가면 렌즈 앞면에 이슬이 맺혀 화면이 뿌옇게 나오는데, 이 현상을 결로라 부른다. → [필수 액세서리 · 앱](../01-gear/accessories.md)
...
- **화이트밸런스 / 켈빈(K)** — 사진의 색온도를 정하는 값. 은하수 촬영은 3800~4200K 부근에서 수동으로 시작한다. → [밤 노출](../02-fundamentals/night-exposure.md)
