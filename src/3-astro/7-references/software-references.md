...
**수록 기준**: 아래 표의 공식 링크는 WebFetch로 직접 접속해 실제로 열리고 해당 소프트웨어의 공식 페이지가 맞는지 확인한 것만 실었습니다(일부는 [촬영 기법별 참고 자료 모음](technique-references.md)에서 이미 검증된 링크를 재사용했습니다 — 표에 별도 표시). 접속에 실패했거나(403/타임아웃 등) 검증하지 못한 항목은 표에 URL을 싣지 않고 마지막 "참고 / 미검증" 절에 따로 정리했습니다.
...
**책에서의 쓰임**: [필수 액세서리 · 앱](../1-gear/accessories.md), [은하수 찾기와 타이밍](../2-fundamentals/finding-the-milkyway.md), [500 / NPF 룰](../2-fundamentals/500-npf-rule.md)

**비고**
- Clear Outside는 공식 사이트를 직접 확인해 "웹 + 무료 iOS/Android 앱" 형태가 맞다는 것과 제작사(First Light Optics)를 확인했습니다.
- Windy는 공식 사이트(windy.com) 접속 자체는 확인했으나, 가져온 페이지 내용이 헤더뿐이라 모바일 앱 유무·가격까지는 이 페이지만으로 재확인하지 못했습니다. 웹 서비스가 무료라는 점은 [촬영 기법별 참고 자료 모음](technique-references.md)에서 이미 확인된 내용을 재사용했습니다.
- Stellarium 데스크톱 버전은 공식 사이트에서 무료·오픈소스임을 재확인했습니다. 다만 페이지에는 모바일(iOS/Android) 유료 버전에 대한 안내가 없어, 모바일 버전의 존재와 가격은 앱스토어에서 직접 확인하는 편이 정확합니다. (책 본문은 "데스크톱 무료, 모바일 유료 버전 있음"이라고 설명합니다.)
- PhotoPills는 photopills.com이 403 Forbidden을 반환해 공식 페이지를 열지 못했습니다 — 아래 미검증 절 참고.
...
**책에서의 쓰임**: [RAW 현상 기본](../5-postprocessing/raw-develop.md), [밤하늘 타임랩스](../6-bonus/timelapse.md)(배치 현상)

**비고**
- 이 책은 darktable을 기본으로 추천하고, Lightroom은 "이미 구독 중이면 그대로 사용"이라는 대안으로 언급합니다.
- Adobe의 공식 제품 페이지(`adobe.com/products/photoshop-lightroom.html`)는 60초 타임아웃으로 접속하지 못했습니다. 플랫폼(Windows/macOS/모바일)과 구독제라는 사실은 일반적으로 잘 알려져 있으나, 공식 페이지로 직접 검증하지는 못했으므로 정확한 현재 가격은 표기하지 않았습니다.
...
| Affinity Photo (by Canva) | **데스크톱(Windows / macOS)** — 천체 스태킹 / iPad — 일반 편집만 | 무료(2025-10 전환, 무료 Canva 계정 필요; 일부 AI 기능만 유료 — 재확인) | **별 정렬 천체 스태킹(Astrophotography Stack)은 데스크톱 전용.** iPad는 RAW·레이어 합성은 되나 별 정렬 스택은 없음 | 공식/앱스토어에서 "Affinity by Canva" 확인 |

**책에서의 쓰임**: [스태킹으로 노이즈 줄이기](../5-postprocessing/stacking.md), 현지 iPad·귀가 PC 구분은 [천체사진 보정 개요](../5-postprocessing/index.md)

**비고**
- 두 도구는 OS로 갈립니다 — Windows면 Sequator(무료), Mac이면 Starry Landscape Stacker(유료)가 사실상 선택지입니다. **무료 대안으로 Affinity Photo 데스크톱**(천체 스태킹 내장)도 있습니다.
- **iPad 주의:** Affinity의 **별 정렬 천체 스태킹은 iPad에 없습니다**(공식 iPad 스태킹 문서에 노출 병합·노이즈 평균·개체 제거·장노출 효과만 있고 별 정렬 항목이 없음). 밤하늘 스태킹은 **데스크톱**(PC/Mac)에서 하고, iPad는 RAW 현상·레이어 합성용으로 쓰세요.
- Affinity는 2025년 10월 Canva가 무료로 전환했다고 알려져 있으나(정확한 무료/유료 구성·플랫폼·URL은 공식 페이지·앱스토어에서 재확인), 천체사진 스태킹 기능(데스크톱) 내장은 확인된 사실입니다.
...
**책에서의 쓰임**: [스타트레일(별궤적) 촬영·합성](../6-bonus/star-trails.md)
...
**책에서의 쓰임**: [밤하늘 타임랩스](../6-bonus/timelapse.md)
...
|---|---|---|---|---|---|
| DeepSkyStacker | Windows (일반적으로 알려진 사실, 공식 페이지에서 재확인 못함) | 무료로 알려짐 (공식 페이지 미검증) | 딥스카이(망원경) 촬영용 스태킹 도구 | 미검증 — 아래 "참고/미검증" 절 참고 | 전경 블렌딩 기능이 없어, 이 책이 다루는 "은하수 + 지평선" 구도에는 적합하지 않음 |
| Topaz Photo AI | Windows 10/11, macOS 12~15 | **구독 전용**(Personal 플랜 등, 정확한 가격은 공식 페이지 확인 필요 — 확인 시 연 $199부터 시작하는 구독형 플랜만 존재) | AI 기반 샤프닝·노이즈 제거·업스케일 | [topazlabs.com/topaz-photo-ai](https://www.topazlabs.com/topaz-photo-ai) | 2025년 10월부터 구독 전용으로 전환되어(공식 페이지에서 구독형 플랜만 확인됨), 한 번 쓰고 마는 여행용·가성비 위주 워크플로에는 맞지 않음 |

**책에서의 쓰임**: [스태킹으로 노이즈 줄이기](../5-postprocessing/stacking.md) — "추천하지 않는 도구" 절

**비고**
- Topaz Photo AI는 공식 페이지(topazlabs.com)를 직접 열어 확인한 결과, 일회성 구매 옵션 없이 구독제(Personal/Pro 플랜, 연 단위 결제 시 $199/년 안팎부터)만 제공하고 있어 책의 "구독 전용 전환" 서술과 일치했습니다. 다만 페이지에 "2025년 10월부터"라는 전환 시점 자체가 명시되어 있지는 않았습니다 — 그 시점은 책 본문의 서술을 그대로 옮긴 것이며, 독립적으로 재확인하지는 못했습니다.
- DeepSkyStacker 공식 페이지(`deepskystacker.free.fr`)는 세 차례 접속을 시도했으나 매번 도구 오류("Command failed with no output")로 내용을 가져오지 못했습니다. Windows 전용·무료라는 점은 일반적으로 널리 알려진 사실이지만, 공식 페이지로 직접 검증하지 못했으므로 URL을 표에 신뢰도 있게 싣지 않았습니다.

---

## 참고 / 미검증

접속·검증에 실패해 표에 공식 링크를 싣지 않은 항목입니다.

- **PhotoPills** — `photopills.com` 전체가 403 Forbidden을 반환해 공식 페이지·앱스토어 링크를 확인하지 못했습니다. ([몽골 천체사진 링크 모음](mongolia-astrophotography-links.md), [촬영 기법별 참고 자료 모음](technique-references.md)에서도 동일 현상이 이미 기록되어 있습니다.) 앱 자체의 존재와 기능(코어 방위·고도 계산, Spot Stars = NPF 계산기)은 다른 사이트를 통해 간접적으로만 확인됩니다. 가격은 앱스토어(App Store/Google Play)에서 직접 확인하세요.
...
- **Adobe Lightroom 공식 제품 페이지** — `adobe.com/products/photoshop-lightroom.html`이 60초 타임아웃으로 접속 실패했습니다(2회 시도). 구독 가격은 Adobe 공식 사이트에서 직접 확인하세요.
- **DeepSkyStacker 공식 페이지** — `deepskystacker.free.fr`이 3회 모두 도구 오류로 내용을 가져오지 못했습니다. 브라우저로 직접 열어 확인하는 편이 낫습니다.
...
카메라·렌즈 장비 추천은 [카메라 · 렌즈 추천 목록](../../appendix/camera-lens-picks.md), 학습용 외부 자료는 [촬영 기법별 참고 자료 모음](technique-references.md) 참고.
