...
- **더 나은 방법 — 국소로 리프트** — 전체 그림자를 올리면 프레임 전체가 탁해집니다. 대신 마스크 **Select Subject**(피사체 선택)로 피사체만 고른 뒤 **노출(Exposure)을 +로** 주면, 배경은 그대로 두고 피사체만 밝힐 수 있습니다. **왜 국소가 나은가:** 국소 리프트는 프레임 전체를 흐리게 만들지 않기 때문입니다. (Select Subject의 실제 조작은 [국소 보정 — 마스킹·디테일](masking-and-detail.md)(Phase 18)에서 다룹니다 — 여기서는 도구 이름만 기억하세요.)
