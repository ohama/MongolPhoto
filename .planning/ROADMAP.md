...
**Success Criteria** (what must be TRUE):
  ...
  2. 2부 목차에 "명소별 영상 스토리보드" 그룹(드론 3: 차강소브라·몽고말떼·어르헝[보너스] + 통합 3: 홍고린엘스·욜링암·바가가즈링촐로)이 스텁 페이지로 나타나고, SUMMARY에 배선되어 `mdbook build src`(create-missing=false)가 exit 0이다.
...
**Goal**: 독자가 2부 "명소별 영상 스토리보드"에서 드론 전용 3종과 드론+지상 통합 3종을 실제 샷 리스트·촬영 설정·동선·편집 흐름·BGM으로 확인하고, 통합 3종은 4부 field-day 세 카메라 오케스트레이션의 실제 적용 사례로 연결된다.
**Depends on**: Phase 29
**Requirements**: SB-01, SB-02
**Success Criteria** (what must be TRUE):
  1. 차강소브라·몽고말떼·어르헝(보너스 라벨, 코스 비편입 명시) 3페이지가 샷 리스트·촬영 설정·동선·편집 흐름·BGM으로 채워지고, 드론 촬영법·CapCut 편집법은 기존 2부 파트로 링크 승계(재설명 없음)한다.
  2. 홍고린엘스·욜링암·바가가즈링촐로 3페이지가 "드론+지상 통합" 라벨로 세 카메라(드론·R7·짐벌/폰) 계획을 담고, 4부 `field-day.md` "세 카메라 오케스트레이션"으로 상향 링크된다.
  3. 4부 `field-day.md`에 통합 스토리보드 3종으로 가는 역링크가 삽입되어 "실제 적용 사례"로 소개된다.
...
**Success Criteria** (what must be TRUE):
  ...
  2. 명소 페이지(1·2·3부의 차강소브라·홍고린엘스·욜링암·바가가즈링촐로)에서 각각 해당 스토리보드로 가는 역링크가 존재한다.
...
| 29. 스토리보드 자산 처리 + 2부 스캐폴드 | v1.6 | 0/? | Not started | - |
