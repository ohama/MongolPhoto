...
Plans:
- [x] 12-01-PLAN.md — index.md(여행 개요·매력·범위) + camera-settings.md(R7 설정 Av↔M·노출보정·측광·CR3 RAW·주간 WB + 렌즈 12mm/50mm/18-150·방진·현장관리 + 초광각/준망원 실전)
- [x] 12-02-PLAN.md — composition-and-light.md(지상 구도·빛 델타·스토리텔링) + landscape-and-street.md(몽골 사람·현지 에티켓 정직표기 + 인물/현장 기초 + 사막 먼지·렌즈 교체 현장관리)
...
**Success Criteria** (what must be TRUE):
  ...
  4. 홍고린엘스 페이지는 사구·낙타 특화, 욜링암 페이지는 협곡·얼음·수염수리·야크·아이벡스(모두 기회성) 특화 팁을 담는다.
  5. 바가가즈링 촐로 페이지는 화강암 기암 특화 팁을 담고, 야생/유목민 요소는 전부 "보이면/기회성"으로 정직 표기되어 있다.
**Plans**: 2 plans

Plans:
- [x] 13-01-PLAN.md — 개요 허브(04-mongolia GPS 표 링크 재사용 + 5곳 안내) + 차강소브라가(층리·황혼) + 바양작(붉은 절벽·화석지 예절)
- [ ] 13-02-PLAN.md — 홍고린엘스(사구·낙타) + 욜링암(슬롯 협곡 12mm 필수·얼음 계절성·야생 기회성) + 바가가즈링 촐로(화강암 기암·유목민 조우 정직 표기)
...
| 13. 코스 명소별 여행 사진 | v1.2 | 0/2 | Planned | - |
