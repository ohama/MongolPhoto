...
**Current focus:** Phase 1 완료·검증(5/5). **2026-07-09 방향 전환** — 2~5부를 개인 일지·여행 결과에서 **일반 가이드**로 재구성(HOME/FIELD/EDIT/SITE). 로드맵·요구사항 재작성 완료. 구 Phase 2/4 산출물은 `.planning/archive/`로 이동. Phase 2(집에서 연습할 사항) 계획·실행 완료(02-01, HOME-01~03). Phase 3(야외에서 연습할 사항) 계획·실행 완료(03-01, FIELD-01~03). Phase 4(후보정 연습) 계획·실행 완료(04-01, EDIT-01~04). 다음: Phase 5(몽골 여행지별 촬영 가이드)는 실제 몽골 여행(8/13~8/17) 이후 착수.
...
- Phase 5 (몽골 여행지별 촬영 가이드, 일반): 계획 대기, 실제 여행(8/13~8/17) 이후 착수 (SITE-01~03) — 아카이브된 몽골 리서치·`src/04-mongolia/` 재활용
...
Progress: [██████░░░░] Phase 1~4 100%, Phase 5~6 재계획 대기 (일반 가이드) — Phase 5는 실제 여행(8/13~8/17) 이후
...
**Recent Trend:**
- Last plans: 02-01 (12 min), 03-01 (7 min), 04-01(신, 후보정 연습, 8 min)
...
- [Phase 5] 일반 "몽골 여행지별 촬영 가이드" 작성 시 `.planning/archive/04-몽골-사전-계획/04-RESEARCH.md`의 검증 데이터(GPS·달·코어)와 기존 `src/04-mongolia/` 페이지를 재활용 — 개인 "Day N / 촬영 결과" 프레이밍만 일반 "명소별 가이드"로 바꾸면 됨. 실제 여행(8/13~8/17) 이후 착수.
